...
class LaundryScene: SKScene {
    ...
    func animateZone(_ zone: SKShapeNode, count: Int, startIndex: Int, originX: CGFloat, color: UIColor) {
        ...
        let centerX = originX + (CGFloat(startIndex) * spriteSpacing) + (width / 2) - (spriteSpacing / 2) // Roughly
    ...
    func updatePointers(low: Int, mid: Int, high: Int) {
         ...
         movePointer(robby, toX: lowX - 20) // Robby sits slightly left of Low index or At Low?
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if redZone.contains(location) { droppedZone = .redSock }
        else if whiteZone.contains(location) { droppedZone = .whiteShirt } // Middle
